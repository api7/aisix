...
//  2. With a MASKING output guardrail the stream is held back until the
//     whole response scans clean, so the caller waits materially longer
//     than the upstream took to start — `downstream_latency_ms` must
//     reflect that wait while `upstream_ttft_ms` must not.
//
...
describe("upstream vs downstream latency split", () => {
  ...
  // NOTE: this test seeds an ENV-SCOPED guardrail, which then applies to
  // every later request in this file — a streamed request would take the
  // hold-back path instead of the verbatim one. It therefore runs LAST;
  // adding a case after it needs a model-scoped attachment instead.
