...
pub async fn create_routers(etcd_prefix: Option<&str>) -> (Router, Router) {
    ...
    let resources =
        Arc::new(aisix::config::entities::ResourceRegistry::new(config_provider.clone()).await);

    let admin_router = aisix::admin::create_router(aisix::admin::AppState::new(
        config.clone(),
        config_provider,
        resources.clone(),
        None,
    ));
    let proxy_router = aisix::proxy::create_router(aisix::proxy::AppState::new(config, resources));
