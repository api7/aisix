...
/// Type-erased span exporter adapter for `init_observability`.
#[derive(Debug)]
...
mod tests {
    ...
    #[rstest::rstest]
    #[case(None)]
    #[case(Some(BoxedSpanExporter::new(
        opentelemetry_otlp::SpanExporter::builder().build().unwrap(),
    )))]
    fn test_boxed_span_exporter(#[case] span_exporter: Option<BoxedSpanExporter>) {
        let _ = match span_exporter {
            Some(exporter) => exporter,
            None => {
                BoxedSpanExporter::new(opentelemetry_otlp::SpanExporter::builder().build().unwrap())
            }
        };
    }
}
