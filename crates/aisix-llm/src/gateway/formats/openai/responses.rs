...
fn responses_text_config_to_hub_response_format(
    text: Option<&crate::types::openai::responses::ResponseTextConfig>,
...
mod tests {
    ...
    impl NativeOpenAIResponsesSupport for DummyResponsesNativeProvider {
        ...
        fn transform_openai_responses_response(
            &self,
            body: Value,
        ) -> crate::error::Result<ResponsesApiResponse> {
        ...
        ) -> crate::error::Result<Vec<ResponsesApiStreamEvent>> {
    ...
    fn from_hub_maps_message_and_tool_calls_to_responses_response() {
        ...
        assert_matches!(&bridged.output[0], crate::types::openai::responses::ResponsesOutputItem::Message { role, .. } if role == "assistant");
        assert_matches!(&bridged.output[1], crate::types::openai::responses::ResponsesOutputItem::FunctionCall { name, .. } if name == "get_weather");
