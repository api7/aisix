macro_rules! provider {
    ...
    (@stream) => {};
    (@stream $kind:expr) => {
        fn stream_reader_kind(&self) -> $crate::traits::StreamReaderKind {
            $kind
        }
    };
    ...
    (@auth bearer) => {
        fn build_auth_headers(
            &self,
            auth: &$crate::provider_instance::ProviderAuth,
        ) -> $crate::error::Result<http::HeaderMap> {
            let mut headers = http::HeaderMap::new();
            let value = http::HeaderValue::from_str(&format!(
                "Bearer {}",
                auth.api_key_for(self.name())?
            ))
                .map_err(|error| $crate::error::GatewayError::Validation(error.to_string()))?;
    ...
    (@quirks { $($field:ident : $value:expr),* $(,)? }) => {
        fn default_quirks(&self) -> $crate::traits::CompatQuirks {
            $crate::traits::CompatQuirks {
                $($field: $value,)*
                ..$crate::traits::CompatQuirks::NONE
            }
        }
    };
    ...
    (@impl_provider
        ...
        impl $crate::traits::ProviderMeta for $name {
        ...
        impl $crate::traits::ProviderCapabilities for $name {}
    };
    ...
    (
        ...
        impl $crate::traits::ChatTransform for $name {
