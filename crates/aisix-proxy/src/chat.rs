...
pub async fn chat_completions(
    ...
    match outcome {
        ...
        Err(failure) => {
            ...
            note_ratelimit_rejection(&state.metrics, &err);
...
/// A rate-limit refusal carries its own counter, on top of the request
/// metrics the failure path's terminal emit records for every error.
fn note_ratelimit_rejection(metrics: &Metrics, err: &ProxyError) {
