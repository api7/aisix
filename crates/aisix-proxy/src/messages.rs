...
fn build_anthropic_sse_stream(
    ...
    let stream = async_stream::stream! {
        ...
        // Model-generated bytes held (#513) — what `max_buffer_bytes` caps.
            ...
            match item {
                Ok(chunk) => {
                    ...
                    if let (Some((max_hold, fail_open)), false) = (hold_policy, released) {
                        ...
                        held_bytes += crate::held_content::chat_delta(&chunk.delta);
    ...
    F: FnOnce(AnthropicStreamUsage) + Send + 'static,
{
    ...
    let inner = async_stream::stream! {
        ...
        // Model-generated bytes in `held` (#513) — what `max_buffer_bytes`
        // caps; the SSE/JSON envelope around them is not counted.
