...
- Renames on the WRITE surface — the DP Admin API and the declarative resources file — converge with `#[serde(alias = "…")]` so existing callers and hand-written files keep working through the deprecation window; never hard-rename a shipped field in one step (an unreleased field with no consumers may rename outright, as #657 did). Regenerate `schemas/` afterwards (`cargo run -p aisix-core --bin dump-schema`, which writes both the strict and the lenient set). A field a projected document carries is a different question — it is never aliased into a new shape here; see the projection rule below.
...
- **Never change a projected field's shape or value domain in place — every release from the support floor up must keep loading the row.** The upgrade order is control plane first, then data planes, and the mixed-version window is bounded by the **floor**, not by time: every etcd document the control plane projects must load as a whole row on every DP release from the floor to the current one. The floor is `dpfloor.Version` in the control-plane repo (`v0.12.0` today); it moves only by explicit decision, never per release. What each of those releases accepts is its lenient read schema, vendored by the control plane per release (a verbatim copy of `schemas/resources-lenient/` from #1137 on, reconstructed from each earlier tag's own producer below that), which validates every projection against every supported release in CI and, at save time, against the versions the registered gateways report — a projection an older gateway cannot load is refused there (`422 DP_INCOMPATIBLE`) instead of reaching etcd. So, from this side: an optional field at any depth and a write-path-only tightening are free (every release at or above the floor opens the read schema all the way down — #1014); a new resource kind costs no DP work either, though on every release below the one that added it each of its rows is a `rejected[]` entry that nothing gates or warns about; a new enum value, or a new variant under an existing `oneOf`, is gated by the control plane at save time and needs nothing here; removing, renaming, or retyping a field an older release requires is banned in place, because a malformed *known* field fails the row and the loader skips it whole. A reshape ships under a NEW field name, and the old one is never reused.
- **This side carries no compat code for a reshape.** While the control plane keeps writing the old key for older gateways, a newer DP treats that key as an ordinary unknown field: the lenient loader accepts the row and reports the path through the partial-compat channel exactly as it does for any unknown field, and filtering that key back out of the report is the control plane's obligation, since it is the side that knows it wrote it (`AISIX-Cloud: internal/cpapi/resources/CLAUDE.md`). No consumed-and-ignored tombstone, no `#[serde(rename = "<old key>")]`: this side carries no time-boxed compatibility code at all and has no marker convention for it — a retired client-facing path is an ordinary deprecation, handled like any other API change. (The DP-emitted `aisix_config_partially_compatible_resources` metric is never filtered, so it counts those deliberately written old keys on up-to-date gateways for as long as the control plane writes them.) Two constraints keep the tolerance real here: unknown-field strictness lives in the strict schema (`models/schema.rs`), never in a nested `#[serde(deny_unknown_fields)]` — the loader deserializes the same types and cannot opt out of a type-level guard; and `serde_ignored` never fires inside serde-buffered content (a `#[serde(flatten)]`ed tagged enum, an untagged variant), so those resources take their report from `unknown_field_paths` instead of loading silently. Whenever new semantics are invisible to an older release, its default there must be fail-closed or no-op; if it would be fail-open, the control plane projects a restrictive old-shape value instead.
