...
- **`total_tokens` on an OpenAI-shape response is recomputed, not echoed from the upstream.** OpenAI clients decompose it as `prompt + completion`; passing through a total built under different accounting breaks that decomposition. An upstream counter we *do* understand belongs in a field of its own instead — Gemini's `thoughtsTokenCount` folds into `completion_tokens` with `reasoning_tokens` naming the subset, rather than inflating a total that no longer adds up.
