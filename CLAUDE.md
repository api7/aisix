...
- **`total_tokens` on an OpenAI-shape response is recomputed, not echoed from the upstream.** OpenAI clients decompose it as `prompt + completion`; passing through a total built under different accounting breaks that decomposition. An upstream counter we *do* understand belongs in a field of its own instead — Gemini's `thoughtsTokenCount` folds into `completion_tokens` with `reasoning_tokens` naming the subset, rather than inflating a total that no longer adds up. That fold is client-facing only: `UsageStats` keeps the folded value for every in-gateway consumer and records the amount in `reasoning_folded_into_completion`, which the UsageEvent builders subtract back out (`recorded_completion_tokens`).
...
- **The UsageEvent's `total_tokens` is `UsageStats::upstream_total_tokens` and nothing else** — the upstream's own total, `0` when it reported none. Never feed it from `total_tokens`, a merge, or any sum built here: cp-api reads `prompt + completion + reasoning == total_tokens` as reasoning billed beside the completion, and stores the value as the upstream's total. A merge (stream frames, ensemble members) keeps it only while every contributor reported one.
...
- Renames on the WRITE surface — the DP Admin API and the declarative resources file — converge with `#[serde(alias = "…")]` so existing callers and hand-written files keep working through the deprecation window; never hard-rename a shipped field in one step (an unreleased field with no consumers may rename outright). Regenerate `schemas/` afterwards (`cargo run -p aisix-core --bin dump-schema`, which writes both the strict and the lenient set). A field a projected document carries is answered by the projection rule below instead: a rename there is a NEW key the control plane writes, never a new alias here. Three aliases are load-bearing exactly because the control plane projects the old key today — `display_name` for `mcp_server.name` and `a2a_agent.name`, `secret` for `provider_key.api_key` — and deleting one makes every row of that collection fail to deserialize, so the loader skips the lot. What an alias must never do is carry a field into a new shape; and no document may carry both keys of an aliased pair, because serde fails the row on the duplicate field and it is skipped whole (`aisix-etcd/src/loader.rs`).
- Exactly four divergence axes are registered as intentional and allowed: reference style (names here vs UUIDs in the CP), tenancy scoping (flat here vs org/environment there), credential custody (`key_hash` in documents here vs server-generated plaintext-once there), and CP-derived fields (`cost`, `telemetry_tags`). Anything else that diverges from cp-admin.yaml is drift. The control plane's contract check (`contract-check.yml`, advisory: nightly and on spec changes there) reports any divergence not registered in its `openapi/cross-plane-allowlist.yaml`.
- Why the CP spec and not this repo's schemas: the CP is spec-first behind a closed validator (its spec already is the authoritative field shape on that side), the spec renders into the customer-facing API reference, and this repo's schemas are generated from the implementation — a schema that follows the implementation cannot lead it.
...
- The per-target invariant (`crates/aisix-proxy/AGENTS.md`: "a per-model gate binds each target") is written around `resolve_attempt_models` — the routing-group trunk. **Ensemble panel/judge (`ProxyModelCaller::call`, the streaming judge) and semantic targets (`semantic::resolve`) bypass that trunk**, so a gate wired only into the trunk is silently absent there. A new per-target gate must be wired into the sub-dispatch paths too, or explicitly deferred with a filed issue. Prefer routing every dispatch through one shared chokepoint so the family can't drift.
- **Strict writes, lenient loads.** `model_one_of` has two variants: the **strict** schema (declarative resources file, the published `schemas/resources/model.schema.json`, every strict validator consumer) forbids a knob a kind never resolves — accepted-but-unread config looks configured and does nothing; the **lenient** loader keeps the base XOR so stored rows written by an older build still load, with `Model::strip_kind_inapplicable` dropping the dead knob and reporting it as `inapplicable:<field>` through the partial-compat channel. The two lists must mirror each other exactly (strict-forbidden ⇔ lenient-stripped) — a field forbidden-but-not-stripped half-honors; stripped-but-not-forbidden vanishes on load while the write path accepts it. A knob is enforced exactly as written or rejected, never half-honored.
- **Never make a field of a projected resource required at the TYPE level.** Requiredness belongs in the strict schema (`require_property` in `models/schema.rs`), never in the struct: the loader validates leniently and then deserializes, and a row it cannot deserialize is **skipped entirely** (`aisix-etcd/src/loader.rs`). Skipping is survivable for a resource the request path treats as optional, but an `api_key` row that fails to load stops authenticating **every** kind of traffic, not just the feature whose field changed — a far worse outcome than the field defaulting. So a new non-`Option` field, or one that loses `#[serde(default)]`, silently turns every already-projected row into a dead one. Give it a serde default whose meaning is fail-closed, and add it to `required` in the strict schema so the write path still refuses to guess. The control plane must also re-emit the affected collection once (`ReprojectMcpAclOnce` is the pattern) — the stored shape changed, but nothing else re-projects a row whose *content* did not.
  - **"In the strict schema" is not automatic — check that the resource HAS a strict/lenient split, and prove the split with a lenient assertion.** A `*_root_schema` that ignores its `strict` flag, or takes none, hands the same `required` list to `LENIENT_SCHEMAS`, and then the requirement you meant for the write path is deleting stored rows instead. A test that reads only the strict schema cannot catch this: it shows a field is required SOMEWHERE and can never show it is required ONLY there. The assertion that pins a split is a document validated against BOTH sets: rejected by `validate_<resource>`, accepted by `validate_<resource>_lenient`.
- **Never change a projected field's shape or value domain in place — every release from the support floor up must keep loading the row.** The upgrade order is control plane first, then data planes, and the mixed-version window is bounded by the **floor**, not by time: every etcd document the control plane projects must load as a whole row on every DP release from the floor to the current one that knows its kind at all — a kind is the one thing a release can predate, and such a row is rejected there until the gateway is upgraded. The floor is `dpfloor.Version` in the control-plane repo (`v0.12.0` as of 2026-09-07); it moves only by explicit decision, never per release. What each of those releases accepts is its lenient read schema, vendored by the control plane per release (a verbatim copy of `schemas/resources-lenient/` from #1137 on, reconstructed from each earlier tag's own producer below that), which validates every projection against every supported release in CI and, at save time, against the versions the registered gateways report — a projection an older gateway cannot load is refused there (`422 DP_INCOMPATIBLE`) instead of reaching etcd. The save-time half judges by the gateways registered at that moment, so an environment whose gateway has not registered yet is written unchecked; the CI half is what holds unconditionally. So, from this side: an optional field at any depth and a write-path-only tightening are free (every release at or above the floor opens the read schema all the way down); a new resource kind costs no DP work either, though it is the exception above — on every release below the one that added it each of its rows is a `rejected[]` entry that nothing refuses or warns about at save time; a new enum value, or a new variant under an existing `oneOf`, is gated by the control plane at save time and needs nothing here; removing, renaming, or retyping a field an older release requires is banned in place, because a malformed *known* field fails the row and the loader skips it whole. A reshape ships under a NEW field name, and the old one is never reused.
- **This side carries no compat code for a reshape.** While the control plane keeps writing the old key for older gateways, a newer DP treats that key as an ordinary unknown field: the lenient loader accepts the row and reports the path through the partial-compat channel exactly as it does for any unknown field, and filtering that key back out of the report is owed by the control plane, the only side that knows it wrote it — until it does, an up-to-date gateway reads as partially compatible for as long as the old key is emitted. No consumed-and-ignored tombstone, no `#[serde(rename = "<old key>")]`: this side carries no time-boxed compatibility code at all and has no marker convention for it — a retired client-facing path is an ordinary deprecation, handled like any other API change. (The DP-emitted `aisix_config_partially_compatible_resources` metric is never filtered at all, whatever the control plane does with its own view.) Two constraints keep the tolerance real here: unknown-field strictness lives in the strict schema (`models/schema.rs`), never in a nested `#[serde(deny_unknown_fields)]` — the loader deserializes the same types and cannot opt out of a type-level guard; and `serde_ignored` never fires inside serde-buffered content (a `#[serde(flatten)]`ed tagged enum, an untagged variant), so those resources take their report from `unknown_field_paths` instead of loading silently. Whenever new semantics are invisible to an older release, its default there must be fail-closed or no-op; if it would be fail-open, the control plane projects a restrictive old-shape value instead.
- **`ensemble` is an experimental surface.** Its known parity gaps — member `allowed_cidrs`/cooldown/health consumption, Prometheus token+spend attribution, response caching, parent-level generic knobs — are deliberate TODOs under a single future design pass. Guardrail scope is not among them: a guardrail attached to a member does not run for group traffic by design, because scope follows the entry the caller addresses (`crates/aisix-proxy/AGENTS.md`). Do not piecemeal-fix one gap ahead of that pass, and do not re-audit them as fresh findings. (The one exception is a marshal-family or shared-chokepoint change where covering ensemble is a one-line parallel edit, e.g. projecting an entry-level field the DP already enforces.)
...
An implicit default would key on the ABSENCE of rows, which is what makes it dangerous: deleting the one model a guardrail was scoped to removes its last attachment, and a zero-attachment default would thereby WIDEN the guardrail to all traffic, silently, on a security control. Do not reintroduce a default of any shape, and in particular do not give the resources file one "for convenience": the file source declares `guardrail_attachments` exactly as the control plane projects them, references written as the identity each collection is keyed by, so both sources answer the scoping question the same way. A convenience default in one of them is a divergence a standalone user cannot see.
...
Say what omitting the block means in `cp-admin.yaml` — the spec the published API reference renders, and the one artifact both planes read. (`cooldown` is the precedent.)
