...
- **Never change a projected field's shape or value domain in place — every release from the support floor up must keep loading the row.** The upgrade order is control plane first, then data planes, and the mixed-version window is bounded by the **floor**, not by time: every etcd document the control plane projects must load as a whole row on every DP release from the floor to the current one that knows its kind at all — a kind is the one thing a release can predate, and such a row is rejected there until the gateway is upgraded. The floor is `dpfloor.Version` in the control-plane repo (`v0.12.0` today); it moves only by explicit decision, never per release. What each of those releases accepts is its lenient read schema, vendored by the control plane per release (a verbatim copy of `schemas/resources-lenient/` from #1137 on, reconstructed from each earlier tag's own producer below that), which validates every projection against every supported release in CI and, at save time, against the versions the registered gateways report — a projection an older gateway cannot load is refused there (`422 DP_INCOMPATIBLE`) instead of reaching etcd. The save-time half judges by the gateways registered at that moment, so an environment whose gateway has not registered yet is written unchecked; the CI half is what holds unconditionally. So, from this side: an optional field at any depth and a write-path-only tightening are free (every release at or above the floor opens the read schema all the way down — #1014); a new resource kind costs no DP work either, though it is the exception above — on every release below the one that added it each of its rows is a `rejected[]` entry that nothing refuses or warns about at save time; a new enum value, or a new variant under an existing `oneOf`, is gated by the control plane at save time and needs nothing here; removing, renaming, or retyping a field an older release requires is banned in place, because a malformed *known* field fails the row and the loader skips it whole. A reshape ships under a NEW field name, and the old one is never reused.
